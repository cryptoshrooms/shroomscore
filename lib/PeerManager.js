...
var shroomscoreDefaults = require('../config');
...
function PeerManager(config) {
  ...
  this.config = config || {};
  for (var i in shroomscoreDefaults)
    if (shroomscoreDefaults.hasOwnProperty(i) && this.config[i] === undefined)
      this.config[i] = shroomscoreDefaults[i];
